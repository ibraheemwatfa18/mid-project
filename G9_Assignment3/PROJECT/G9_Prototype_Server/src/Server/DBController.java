...
public class DBController {
    ...
    /**
     * resolves a visitor's contact email at query time. {@code orders.email} was removed for BCNF
     * normalization (email is functionally determined by {@code visitor_id} and lives authoritatively
     * in the visitors / subscribers / guides tables), so order SELECTs derive it instead of storing it.
     * drop this expression into any order query that aliases the orders table as {@code o}; it yields
     * the same {@code email} column the result-mapping code already reads. lookup priority matches
     * {@link #getVisitorEmail}: visitor → subscriber → guide, falling back to an empty string.
     */
    private static final String ORDER_EMAIL_SUBQUERY =
        "COALESCE((SELECT email FROM visitors    WHERE id_number = o.visitor_id), " +
        "         (SELECT email FROM subscribers WHERE id_number = o.visitor_id), " +
        "         (SELECT email FROM guides      WHERE id_number = o.visitor_id), '')";
    ...
    /**
     * returns how many spots a <em>walk-in</em> may use at the gate right now.
     *
     * <p>walk-ins are measured against the park's <b>physical {@code capacity}</b> (e.g. 100), not
     * the lower pre-booking {@code max_orders} cap (e.g. 90). this is the gap that gives walk-ins
     * the buffer: reservations can only ever fill up to {@code max_orders}, leaving
     * {@code capacity − max_orders} physical spots that walk-ins can still claim — on top of any
     * unused booking spots. so with 86 of 90 reserved, a walk-in sees {@code 100 − 86 = 14} free.
     *
     * <p>occupancy counts two disjoint groups so the physical limit is genuinely enforced (and
     * back-to-back walk-ins can't blow past capacity):
     * <ul>
     *   <li>pending/confirmed reservations whose visit-duration window overlaps {@code visitTime}, and</li>
     *   <li>visitors physically inside right now (status {@code completed}, checked in, not yet exited).</li>
     * </ul>
     * an order is in exactly one status, so the two branches never double-count the same row.
     *
     * @param parkId    the park's primary-key ID
     * @param visitDate the date (today, {@code yyyy-MM-dd})
     * @param visitTime the walk-in arrival time in {@code HH:mm} format
     * @return free physical spots for a walk-in, or {@code -1} on DB error or unknown park
     */
    public int getWalkInAvailableSpots(int parkId, String visitDate, String visitTime) {
        if (connection == null) return -1;
        try (PreparedStatement st = connection.prepareStatement(
                "SELECT p.capacity - COALESCE(SUM(occ.num_visitors), 0) AS available " +
                "FROM parks p " +
                "LEFT JOIN orders occ " +
                "       ON  occ.park_id = p.id " +
                "       AND occ.visit_date = ? " +
                "       AND ( " +
                "             ( occ.status IN ('confirmed','pending') " +
                "               AND TIME(occ.visit_time) < ADDTIME(TIME(?), SEC_TO_TIME(p.visit_duration_hours * 3600)) " +
                "               AND ADDTIME(TIME(occ.visit_time), SEC_TO_TIME(p.visit_duration_hours * 3600)) > TIME(?) ) " +
                "          OR ( occ.status = 'completed' AND occ.entry_time IS NOT NULL AND occ.exit_time IS NULL ) " +
                "       ) " +
                "WHERE p.id = ? " +
            ...
            try (ResultSet rs = st.executeQuery()) {
                if (rs.next()) {
                    int result = rs.getInt("available");
                    if (result < 0) result = 0;   // overbooked clamps to 0, not the -1 error sentinel
                    System.out.printf("[getWalkInAvailableSpots] parkId=%d time=%s result=%d%n",
                        parkId, visitTime, result);
                    return result;
                }
            }
            return -1;   // no row → park not found
        } catch (SQLException e) {
            System.out.println("[getWalkInAvailableSpots] SQL ERROR: " + e.getMessage());
            return -1;
        }
    }
    ...
    public int checkAndBook(BookingRequest req) throws SQLException {
        ...
        try {
            ...
            // Reject duplicate bookings: a visitor cannot hold two active bookings whose
            // time windows overlap, even across different parks — they can only be in one
            // place at a time.  We join orders with parks to get each existing booking's
            // duration so the overlap test is accurate regardless of which park was booked.
            // The new booking's window is [newTime, newTime + durationHours).
            // An existing booking at existingTime with existingDuration overlaps when:
            //   existingTime < newTime + newDuration  AND  existingTime + existingDuration > newTime
            try (PreparedStatement st = connection.prepareStatement(
                    "SELECT COUNT(*) AS cnt " +
                    "FROM orders o " +
                    ...
                    "  AND o.visit_date = ? " +
                    "  AND o.status IN ('confirmed', 'pending') " +
                    "  AND TIME(o.visit_time) < ADDTIME(TIME(?), SEC_TO_TIME(? * 3600)) " +
                    "  AND ADDTIME(TIME(o.visit_time), SEC_TO_TIME(p.visit_duration_hours * 3600)) > TIME(?)")) {
                st.setString(1, req.getVisitorId());
                st.setString(2, req.getVisitDate());
                st.setString(3, req.getVisitTime());
                st.setInt(4,    durationHours);
                st.setString(5, req.getVisitTime());
                try (ResultSet rs = st.executeQuery()) {
                    if (rs.next() && rs.getInt("cnt") > 0) {
                        connection.rollback();
                        System.out.printf("[checkAndBook] duplicate rejected — visitor %s already has "
                            + "an overlapping booking on %s%n", req.getVisitorId(), req.getVisitDate());
                        return -2;
                    }
                }
            }

            // All checks passed; insert the confirmed order.
            String orderTypeValue = toDbOrderType(req.getOrderType());
            int orderId;
            try (PreparedStatement st = connection.prepareStatement(
                    "INSERT INTO orders " +
                    "(visitor_id, park_id, visit_date, visit_time, num_visitors, order_type, status) " +
                    "VALUES (?, ?, ?, ?, ?, ?, 'confirmed')",
    ...
    /**
     * finds the 3 nearest slots to the requested date/time that this visitor can
     * actually book for the same park.
     *
     * <p>candidate slots are every standard hourly slot (08:00–16:00) over the next 7
     * days, ordered by absolute distance from the requested date/time so the closest
     * openings are offered first. each candidate must pass <b>both</b> of these checks,
     * which together guarantee that selecting it results in a confirmed booking:
     * <ul>
     *   <li><b>park capacity</b> — overlapping confirmed/pending visitors plus
     *       {@code numVisitors} must not exceed {@code max_orders};</li>
     *   <li><b>no self-overlap</b> — the visitor must not already hold a confirmed or
     *       pending booking whose time window overlaps the candidate slot (mirrors the
     *       duplicate-booking guard in {@link #checkAndBook}, which would otherwise
     *       reject the booking).</li>
     * </ul>
     *
     * @param parkId      the park to check
     * @param visitDate   the originally requested date in {@code yyyy-MM-dd} format
     * @param visitTime   the originally requested time in {@code HH:mm} format (excluded)
     * @param numVisitors the party size that must fit in the alternative slot
     * @param visitorId   the requesting visitor's ID, used for the self-overlap check
     * @return up to 3 bookable alternative slots as {@code "yyyy-MM-dd HH:mm"} strings,
     *         nearest first; empty if none are available within the 7-day window
     */
    public java.util.List<String> findAlternativeSlots(
            int parkId, String visitDate, String visitTime, int numVisitors, String visitorId) {

        java.util.List<String> result = new java.util.ArrayList<>();
        if (connection == null) return result;

        // All standard time slots the booking form offers.
        String[] allSlots = {
            "08:00", "09:00", "10:00", "11:00", "12:00",
            "13:00", "14:00", "15:00", "16:00"
        };

        // Fetch the park's limits once; the duration is the same for every candidate.
        int maxOrders, durationHours;
        try (PreparedStatement st = connection.prepareStatement(
                "SELECT max_orders, visit_duration_hours FROM parks WHERE id = ?")) {
            st.setInt(1, parkId);
            try (ResultSet rs = st.executeQuery()) {
                if (!rs.next()) return result;   // park not found
                maxOrders     = rs.getInt("max_orders");
                durationHours = rs.getInt("visit_duration_hours");
            }
        } catch (SQLException e) {
            System.out.println("findAlternativeSlots (park lookup) error: " + e.getMessage());
            return result;
        }

        // Build every (date, time) candidate over the next 7 days and order them by how
        // close they fall to the originally requested moment, nearest first.  The exact
        // requested slot itself is skipped (it's the full one).
        java.time.LocalDateTime requested;
        try {
            requested = java.time.LocalDateTime.of(
                java.time.LocalDate.parse(visitDate),
                java.time.LocalTime.parse(visitTime));
        } catch (Exception e) {
            return result;   // unparseable input; nothing to suggest
        }

        java.util.List<java.time.LocalDateTime> candidates = new java.util.ArrayList<>();
        java.time.LocalDate baseDate = requested.toLocalDate();
        for (int dayOffset = 0; dayOffset < 7; dayOffset++) {
            java.time.LocalDate d = baseDate.plusDays(dayOffset);
            for (String slot : allSlots) {
                java.time.LocalDateTime cand = java.time.LocalDateTime.of(
                    d, java.time.LocalTime.parse(slot));
                if (cand.equals(requested)) continue;       // skip the full slot itself
                if (cand.isBefore(java.time.LocalDateTime.now())) continue;  // no past slots
                candidates.add(cand);
            }
        }
        final java.time.LocalDateTime ref = requested;
        candidates.sort(java.util.Comparator.comparingLong(
            c -> Math.abs(java.time.Duration.between(ref, c).toMinutes())));

        // Capacity check for one candidate slot.
        String capacitySql =
            "SELECT COALESCE(SUM(o.num_visitors), 0) AS occupied " +
            "FROM orders o " +
            "WHERE o.park_id = ? AND o.visit_date = ? " +
            "  AND o.status IN ('confirmed', 'pending') " +
            "  AND TIME(o.visit_time) < ADDTIME(TIME(?), SEC_TO_TIME(? * 3600)) " +
            "  AND ADDTIME(TIME(o.visit_time), SEC_TO_TIME(? * 3600)) > TIME(?)";

        // Self-overlap check: does this visitor already hold an overlapping booking
        // (any park) on the candidate date?  Joins parks to use each existing booking's
        // own duration, exactly as the duplicate guard in checkAndBook does.
        String selfSql =
            "SELECT COUNT(*) AS cnt " +
            "FROM orders o JOIN parks p ON p.id = o.park_id " +
            "WHERE o.visitor_id = ? AND o.visit_date = ? " +
            "  AND o.status IN ('confirmed', 'pending') " +
            "  AND TIME(o.visit_time) < ADDTIME(TIME(?), SEC_TO_TIME(? * 3600)) " +
            "  AND ADDTIME(TIME(o.visit_time), SEC_TO_TIME(p.visit_duration_hours * 3600)) > TIME(?)";

        try (PreparedStatement capSt  = connection.prepareStatement(capacitySql);
             PreparedStatement selfSt = connection.prepareStatement(selfSql)) {
            for (java.time.LocalDateTime cand : candidates) {
                if (result.size() >= 3) break;
                String dateStr = cand.toLocalDate().toString();                 // yyyy-MM-dd
                String timeStr = String.format("%02d:%02d",
                    cand.getHour(), cand.getMinute());                          // HH:mm

                // 1) park capacity
                capSt.setInt(1,    parkId);
                capSt.setString(2, dateStr);
                capSt.setString(3, timeStr);
                capSt.setInt(4,    durationHours);
                capSt.setInt(5,    durationHours);
                capSt.setString(6, timeStr);
                int occupied = 0;
                try (ResultSet rs = capSt.executeQuery()) {
                    if (rs.next()) occupied = rs.getInt("occupied");
                }
                if (occupied + numVisitors > maxOrders) continue;   // would be full

                // 2) visitor's own overlap
                selfSt.setString(1, visitorId);
                selfSt.setString(2, dateStr);
                selfSt.setString(3, timeStr);
                selfSt.setInt(4,    durationHours);
                selfSt.setString(5, timeStr);
                int selfConflicts = 0;
                try (ResultSet rs = selfSt.executeQuery()) {
                    if (rs.next()) selfConflicts = rs.getInt("cnt");
                }
                if (selfConflicts > 0) continue;   // visitor already booked over this slot

                result.add(dateStr + " " + timeStr);
            }
        } catch (SQLException e) {
            System.out.println("findAlternativeSlots error: " + e.getMessage());
        }
        return result;
    }
    ...
    /**
     * declines a promoted waitlist offer at the visitor's own request, before the 1-hour window
     * expires. reuses the exact release path the timeout uses ({@link #expireWaitlistEntry}): the
     * held PENDING order is cancelled, the visitor's waiting-list row is removed, and the freed slot
     * is offered to the next eligible visitor in line (starting their own 1-hour window). idempotent —
     * if the offer is no longer live (already confirmed, expired, or handled), nothing changes.
     *
     * @param orderId the held promotion order the visitor is declining
     * @return {@code true} if the spot was released and re-offered to the next visitor;
     *         {@code false} if it was no longer a live NOTIFIED promotion
     */
    public synchronized boolean declineWaitlistPromotion(int orderId) {
        if (connection == null) return false;
        int wlId;
        try (PreparedStatement st = connection.prepareStatement(
                "SELECT id FROM waiting_list WHERE order_id = ? AND status = 'NOTIFIED'")) {
            st.setInt(1, orderId);
            ResultSet rs = st.executeQuery();
            if (!rs.next()) return false;   // not a live promotion offer
            wlId = rs.getInt("id");
        } catch (SQLException e) {
            System.out.println("declineWaitlistPromotion (lookup) error: " + e.getMessage());
            return false;
        }
        PromotedEntry e = getNotifiedEntryById(wlId);
        if (e == null) return false;        // raced with a confirm or the timeout sweep
        // same proven release path as the 1-hour timeout: cancel held order, drop the
        // waiting-list row, and promote the next eligible visitor in line.
        return expireWaitlistEntry(e);
    }

    /**
     * marks a PENDING order as CONFIRMED; used when a visitor confirms an upcoming visit
     * through the day-before reminder path.
     *
     * <p>also deletes any orphaned {@code waiting_list} row linked to this order.
     * this can happen when a visitor confirms a waitlist-sourced booking via the
     * ordinary reminder flow rather than the dedicated waitlist-promotion dialog:
     * {@code confirmPromotedVisit} is never called, so the {@code WAITING} row would
     * otherwise remain indefinitely, leaving the two tables inconsistent.
     *
     * <p>only rows currently {@code 'pending'} are changed; already-confirmed, cancelled, or
    ...
    public boolean confirmOrder(int orderId) {
        ...
        try (PreparedStatement st = connection.prepareStatement(
            ...
            // clean up any waiting_list row linked to this order (WAITING or NOTIFIED)
            // so the two tables stay consistent after confirmation via either path
            try (PreparedStatement del = connection.prepareStatement(
                    "DELETE FROM waiting_list WHERE order_id = ?")) {
                del.setInt(1, orderId);
                del.executeUpdate();
            } catch (SQLException e) {
                System.out.println("confirmOrder (waiting_list cleanup) error: " + e.getMessage());
            }
            return updated;
        } catch (SQLException e) {
     ...
     *
     * @param visitorId the visitor's government ID to look up
     * @param parkId    the park the visitor is trying to enter
     * @return the matching {@link OrderDetail} for today, or {@code null} if none exists
     */
    public OrderDetail getTodayBooking(String visitorId, int parkId) {
        ...
        try {
            PreparedStatement st = connection.prepareStatement(
                ...
                "       o.visit_date, o.visit_time, o.status, " +
                "       " + ORDER_EMAIL_SUBQUERY + " AS email, o.created_at, " +
                "       p.name AS park_name " +
                "FROM orders o JOIN parks p ON p.id = o.park_id " +
    ...
    /**
     * finds today's pending/confirmed booking by its order number (the "ORDER-xxx" reference shown
     * on the entry QR pass), scoped to the given park. lets the gate look a visitor up by the short
     * booking reference instead of the 9-digit government ID. mirrors {@link #getTodayBooking}.
     *
     * @param orderId the booking's order number
     * @param parkId  the park the employee is stationed at
     * @return the matching {@link OrderDetail}, or {@code null} if none today at this park
     */
    public OrderDetail getTodayBookingByOrderId(int orderId, int parkId) {
        if (connection == null) return null;
        try {
            PreparedStatement st = connection.prepareStatement(
                "SELECT o.id, o.visitor_id, o.num_visitors, o.order_type, " +
                ...
                "       p.name AS park_name " +
                "FROM orders o JOIN parks p ON p.id = o.park_id " +
                "WHERE o.id=? AND o.park_id=? AND o.visit_date=CURDATE() " +
                "  AND o.status IN ('pending','confirmed') " +
                "LIMIT 1");
            st.setInt(1, orderId);
            st.setInt(2, parkId);
            ...
            System.out.println("getTodayBookingByOrderId error: " + e.getMessage());
            return null;
        }
    }
    ...
    public EntryResult checkInVisitor(EntryCheckRequest req) {
        ...
        // walk-ins are bounded by the park's PHYSICAL capacity, not the lower booking cap, so they
        // can use the capacity − max_orders buffer that reservations are never allowed to fill.
        int    available = getWalkInAvailableSpots(parkId, today, curTime);
        ...
        try {
            ...
            try (PreparedStatement st = connection.prepareStatement(
                    "INSERT INTO orders " +
                    "(visitor_id, park_id, visit_date, visit_time, num_visitors, order_type, status, entry_time) " +
    ...
    /**
     * registers a group's exit by its order number (the "ORDER-xxx" reference on the entry pass),
     * scoped to the given park. stamps {@code exit_time} on the checked-in order. mirrors
     * {@link #registerExit(String, int)} but keyed on the booking reference instead of the ID.
     *
     * @param orderId the booking's order number
     * @param parkId  the park the employee is stationed at
     * @return {@code true} if an active (checked-in, not-yet-exited) order was stamped out
     */
    public boolean registerExitByOrderId(int orderId, int parkId) {
        if (connection == null) return false;
        try (PreparedStatement st = connection.prepareStatement(
                "UPDATE orders SET exit_time=NOW() " +
                "WHERE id=? AND park_id=? AND visit_date=CURDATE() " +
                "  AND status='completed' AND entry_time IS NOT NULL AND exit_time IS NULL " +
                "LIMIT 1")) {
            st.setInt(1, orderId);
            st.setInt(2, parkId);
            ...
            System.out.println("registerExitByOrderId error: " + e.getMessage());
            return false;
        }
    }
    ...
    /**
     * usage report: average <b>concurrent occupancy</b> per hour slot (08:00–18:00) for the given
     * calendar month, per park. the client divides by capacity × 100 to get percentage-of-capacity
     * for the line chart — i.e. how close the park came to its limit, so an operator can see WHEN it
     * was under capacity (per the spec's דו"ח שימוש requirement).
     *
     * <p><b>Interval-overlap occupancy:</b> a visitor is counted in every hour H that their stay
     * spans, not just their arrival hour — a visit with {@code HOUR(entry_time) <= H < HOUR(exit_time)}
     * is "inside" during H. This is computed by cross-joining each order against an hours list (8–18)
     * on that overlap condition, so a 4-hour visit contributes to four hour buckets and midday
     * occupancy correctly accumulates across staggered arrivals.
     *
     * <p><b>Denominator:</b> each hour's monthly visitor total is divided by the number of distinct
     * dates that hour actually had someone present ({@code COUNT(DISTINCT visit_date)} after the
     * overlap join), giving the average concurrent occupancy on a day that hour was in use.
     *
    ...
    public List<ReportUsageRow> getUsageReport(int year, int month) {
        ...
        try {
            PreparedStatement st = connection.prepareStatement(
                ...
                "       CONCAT(LPAD(h.hour_slot, 2, '0'), ':00') AS hour_slot, " +
                "       p.capacity, " +
                "       ROUND(SUM(o.num_visitors) / COUNT(DISTINCT o.visit_date), 1) AS avg_per_day " +
                "FROM (SELECT 8 AS hour_slot UNION SELECT 9 UNION SELECT 10 UNION SELECT 11 " +
                "      UNION SELECT 12 UNION SELECT 13 UNION SELECT 14 UNION SELECT 15 " +
                "      UNION SELECT 16 UNION SELECT 17 UNION SELECT 18) h " +
                "JOIN orders o ON HOUR(o.entry_time) <= h.hour_slot AND h.hour_slot < HOUR(o.exit_time) " +
                "JOIN parks p ON p.id = o.park_id " +
                "WHERE o.status = 'completed' " +
                "  AND o.entry_time IS NOT NULL AND o.exit_time IS NOT NULL " +
                "  AND YEAR(o.visit_date) = ? AND MONTH(o.visit_date) = ? " +
                "GROUP BY p.id, p.name, p.capacity, h.hour_slot " +
    ...
    /**
     * average-visit-duration report: the mean length of stay (in hours) for each visitor
     * type during the given calendar month, across all parks.
     *
     * <p>only orders that recorded both an {@code entry_time} and an {@code exit_time}
     * contribute, so in-progress and future visits never skew the average. each visit's
     * duration is {@code TIMESTAMPDIFF(MINUTE, entry_time, exit_time) / 60.0} hours, and the
     * {@code order_type} column is folded into two buckets — {@code 'group'} becomes
     * {@code "Group"}, everything else ({@code 'individual'}, {@code 'family'}) becomes
     * {@code "Individual"} — to match the "individual vs organized group" split the
     * department-manager report calls for.
     *
     * @param year  the four-digit calendar year
     * @param month the month number 1-12
     * @return a list of {@link ReportDurationRow} ordered by visitor type; empty on error or
     *         when the period has no completed visits
     */
    public List<ReportDurationRow> getVisitDurationByType(int year, int month) {
        List<ReportDurationRow> list = new ArrayList<>();
        if (connection == null) return list;
        try (PreparedStatement st = connection.prepareStatement(
                "SELECT CASE WHEN LOWER(o.order_type) = 'group' THEN 'Group' " +
                "            ELSE 'Individual' END AS visitor_type, " +
                "       ROUND(AVG(TIMESTAMPDIFF(MINUTE, o.entry_time, o.exit_time) / 60.0), 2) " +
                "         AS avg_hours " +
                "FROM orders o " +
                "WHERE o.entry_time IS NOT NULL AND o.exit_time IS NOT NULL " +
                "  AND YEAR(o.visit_date) = ? AND MONTH(o.visit_date) = ? " +
                "GROUP BY visitor_type " +
                "ORDER BY visitor_type ASC")) {
            st.setInt(1, year);
            st.setInt(2, month);
            try (ResultSet rs = st.executeQuery()) {
                while (rs.next()) {
                    list.add(new ReportDurationRow(
                        rs.getString("visitor_type"),
                        rs.getDouble("avg_hours")));
                }
            }
        } catch (SQLException e) {
            System.out.println("getVisitDurationByType error: " + e.getMessage());
        }
        return list;
    }
    ...
    /**
     * returns ALL pending/confirmed orders for tomorrow so the server UI can display them
     * on every startup. each entry carries a {@code reminderSent} flag so
     * {@link ReminderService} can skip re-sending emails for orders already reminded.
     *
    ...
    /**
     * marks an order as reminded so subsequent server starts skip it.
     * called immediately after each reminder email is sent.
     *
     * @param orderId the order to mark
     */
    public void markReminderSent(int orderId) {
        ...
        try {
            ...
            System.out.println("markReminderSent error (order #" + orderId + "): " + e.getMessage());
        }
    }
    ...
    /**
     * loads a person's editable personal details by ID for the "My Details" view and the
     * Service Rep's view/edit panel. resolves the richest profile available: subscriber first
     * (carries credit card + family size), then guide, then plain visitor.
     *
     * @param idNumber the person's government ID
     * @return a {@link PersonDetails}, or {@code null} if the ID isn't registered or on DB error
     */
    public PersonDetails getPersonDetails(String idNumber) {
        if (connection == null || idNumber == null) return null;
        // Subscriber — richest profile (unique member number + credit card + family size)
        try (PreparedStatement st = connection.prepareStatement(
                "SELECT subscriber_id, first_name, last_name, email, phone, " +
                "       COALESCE(credit_card,'') AS credit_card, family_size " +
                "FROM subscribers WHERE id_number = ?")) {
            st.setString(1, idNumber);
            ResultSet rs = st.executeQuery();
            if (rs.next())
                return new PersonDetails(idNumber, rs.getString("first_name"),
                    rs.getString("last_name"), rs.getString("email"), rs.getString("phone"),
                    rs.getString("credit_card"), rs.getInt("family_size"), "SUBSCRIBER", true,
                    rs.getInt("subscriber_id"));
        } catch (SQLException e) {
            System.out.println("getPersonDetails (subscriber) error: " + e.getMessage());
        }
        // Guide
        try (PreparedStatement st = connection.prepareStatement(
                "SELECT first_name, last_name, email, phone FROM guides WHERE id_number = ?")) {
            st.setString(1, idNumber);
            ResultSet rs = st.executeQuery();
            if (rs.next())
                return new PersonDetails(idNumber, rs.getString("first_name"),
            ...
            System.out.println("getPersonDetails (guide) error: " + e.getMessage());
        }
        // Plain visitor
        try (PreparedStatement st = connection.prepareStatement(
                "SELECT first_name, last_name, email, phone FROM visitors WHERE id_number = ?")) {
            st.setString(1, idNumber);
            ResultSet rs = st.executeQuery();
            if (rs.next())
                return new PersonDetails(idNumber, rs.getString("first_name"),
            ...
            System.out.println("getPersonDetails (visitor) error: " + e.getMessage());
        }
        return null;
    }

    /**
     * updates a person's personal details across every table that holds a copy. the base
     * {@code visitors} row is updated for all registered people; the {@code subscribers} and
     * {@code guides} rows are updated too when the ID exists there (the schema denormalizes
     * name/email/phone into those tables). all updates run in one transaction so a failure rolls
     * back together. the ID number itself is never changed — it is the immutable key.
     *
     * @param d the new details (ID identifies the row; credit card / family size apply to subscribers)
     * @return {@code true} if at least the base visitor/subscriber row was updated; {@code false}
     *         if the ID wasn't found anywhere or on DB error
     */
    public synchronized boolean updatePersonDetails(PersonDetails d) {
        if (connection == null || d == null || d.getIdNumber() == null) return false;
        boolean oldAuto;
        try { oldAuto = connection.getAutoCommit(); }
        catch (SQLException e) { return false; }
        try {
            connection.setAutoCommit(false);
            boolean any = false;

            // Base visitors row (exists for every registered person)
            try (PreparedStatement st = connection.prepareStatement(
                    "UPDATE visitors SET first_name=?, last_name=?, email=?, phone=? " +
                    "WHERE id_number=?")) {
                st.setString(1, d.getFirstName()); st.setString(2, d.getLastName());
                st.setString(3, d.getEmail());     st.setString(4, d.getPhone());
                st.setString(5, d.getIdNumber());
                any |= st.executeUpdate() > 0;
            }

            // Subscriber row (richer profile: credit card + family size)
            try (PreparedStatement st = connection.prepareStatement(
                    "UPDATE subscribers SET first_name=?, last_name=?, email=?, phone=?, " +
                    "credit_card=?, family_size=? WHERE id_number=?")) {
                st.setString(1, d.getFirstName()); st.setString(2, d.getLastName());
                st.setString(3, d.getEmail());     st.setString(4, d.getPhone());
                if (d.getCreditCard() != null && !d.getCreditCard().isEmpty())
                    st.setString(5, d.getCreditCard());
                else
                    st.setNull(5, java.sql.Types.VARCHAR);
                st.setInt(6, d.getFamilySize() > 0 ? d.getFamilySize() : 1);
                st.setString(7, d.getIdNumber());
                any |= st.executeUpdate() > 0;
            }

            // Guide row (name/email/phone copy)
            try (PreparedStatement st = connection.prepareStatement(
                    "UPDATE guides SET first_name=?, last_name=?, email=?, phone=? " +
                    "WHERE id_number=?")) {
                st.setString(1, d.getFirstName()); st.setString(2, d.getLastName());
                st.setString(3, d.getEmail());     st.setString(4, d.getPhone());
                st.setString(5, d.getIdNumber());
            ...
            System.out.println("updatePersonDetails error: " + e.getMessage());
            try { connection.rollback(); } catch (SQLException ignored) {}
            return false;
        } finally {
    ...
    public String updateOrder(int orderId, String newDate, int newVisitors, String orderType) {
        ...
        try {
            // Fetch the order's current status, visitor, park, time, and the park's capacity/
            // duration so we can validate the reschedule before applying it. The visit TIME and
            // PARK are not changed by a reschedule — only the date and visitor count — so the
            // booking window stays [visit_time, visit_time + duration).
            PreparedStatement chk = connection.prepareStatement(
                "SELECT o.status, o.visitor_id, o.park_id, " +
                "       TIME_FORMAT(o.visit_time, '%H:%i') AS visit_time, " +
                "       p.max_orders, p.visit_duration_hours " +
                "FROM orders o JOIN parks p ON p.id = o.park_id WHERE o.id=?");
            ...
            int    durationHours = rs.getInt("visit_duration_hours");
            ...
            // ── Capacity check on the NEW date ──────────────────────────────────────────
            // Sum visitors of overlapping pending/confirmed orders on the new date at this park,
            // EXCLUDING this order itself (so moving within the same date doesn't block itself).
            try (PreparedStatement cap = connection.prepareStatement(
                    "SELECT COALESCE(SUM(num_visitors), 0) AS occupied FROM orders " +
                    "WHERE park_id = ? AND visit_date = ? AND status IN ('confirmed','pending') " +
                    "  AND id <> ? " +
                    "  AND TIME(visit_time) < ADDTIME(TIME(?), SEC_TO_TIME(? * 3600)) " +
                    "  AND ADDTIME(TIME(visit_time), SEC_TO_TIME(? * 3600)) > TIME(?)")) {
                cap.setInt(1, parkId);
                cap.setString(2, newDate);
                cap.setInt(3, orderId);
                cap.setString(4, visitTime);
                cap.setInt(5, durationHours);
                cap.setInt(6, durationHours);
                cap.setString(7, visitTime);
                ResultSet capRs = cap.executeQuery();
                int occupied = capRs.next() ? capRs.getInt("occupied") : 0;
                if (occupied + newVisitors > maxOrders) {
                    int free = Math.max(0, maxOrders - occupied);
                    return "That date is full at " + visitTime + " — only " + free
                         + " spot(s) left, but this booking needs " + newVisitors + ".";
                }
            }

            // ── Overlap check ───────────────────────────────────────────────────────────
            // The visitor can't hold two overlapping bookings (can't be in two places at once);
            // check their OTHER active orders on the new date, excluding this one.
            try (PreparedStatement ov = connection.prepareStatement(
                    "SELECT COUNT(*) AS cnt FROM orders o JOIN parks p ON p.id = o.park_id " +
                    "WHERE o.visitor_id = ? AND o.visit_date = ? AND o.status IN ('confirmed','pending') " +
                    "  AND o.id <> ? " +
                    "  AND TIME(o.visit_time) < ADDTIME(TIME(?), SEC_TO_TIME(? * 3600)) " +
                    "  AND ADDTIME(TIME(o.visit_time), SEC_TO_TIME(p.visit_duration_hours * 3600)) > TIME(?)")) {
                ov.setString(1, visitorId);
                ov.setString(2, newDate);
                ov.setInt(3, orderId);
                ov.setString(4, visitTime);
                ov.setInt(5, durationHours);
                ov.setString(6, visitTime);
                ResultSet ovRs = ov.executeQuery();
                if (ovRs.next() && ovRs.getInt("cnt") > 0)
                    return "Visitor " + visitorId + " already has another booking that overlaps "
                         + newDate + " at " + visitTime + ". A visitor can only be in one place at a time.";
            }
    ...
    /**
     * returns a single visitor's own still-waiting entries (across all parks) so they can be
     * shown in the visitor's "My Orders" screen with a status of {@code "WAITING"}.
     *
     * <p>only entries that are genuinely WAITING are returned: a row that has already been
     * promoted (status {@code NOTIFIED}) is excluded here, because that case is handled by the
     * dedicated confirm/decline dialog rather than the ordinary "leave the waiting list" action.
     * mirrors {@link #getWaitingList(int)} but filters by {@code visitor_id} instead of park.
     *
     * @param visitorId the visitor's government ID
     * @return a list of {@link OrderDetail} with status {@code "WAITING"}; empty if none or on error
     */
    public List<OrderDetail> getVisitorWaitingEntries(String visitorId) {
        List<OrderDetail> list = new ArrayList<>();
        if (connection == null) return list;
        try {
            PreparedStatement st = connection.prepareStatement(
                ...
                "       DATE_FORMAT(wl.created_at, '%Y-%m-%d %H:%i') AS created_at " +
                "FROM waiting_list wl JOIN parks p ON p.id = wl.park_id " +
                "WHERE wl.visitor_id = ? AND COALESCE(wl.status, 'WAITING') = 'WAITING' " +
                "ORDER BY wl.visit_date, wl.visit_time, wl.created_at");
            st.setString(1, visitorId);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                String vid = rs.getString("visitor_id");
                list.add(new OrderDetail(
                    rs.getInt("id"),          rs.getString("park_name"),
                    vid,                        rs.getString("visit_date"),
                    rs.getString("visit_time"), rs.getInt("num_visitors"),
                    rs.getString("order_type"), rs.getString("status"),
                    getVisitorEmail(vid),       rs.getString("created_at")));
            }
        } catch (SQLException e) {
            System.out.println("getVisitorWaitingEntries error: " + e.getMessage());
        }
        return list;
    }

    /**
     * removes a visitor's own still-waiting entry from the waiting list ("leave the queue").
     *
     * <p>guarded two ways so it can never delete the wrong row: the {@code visitor_id} must match
     * the requester (you can only leave your own entry), and the status must still be
     * {@code WAITING} — a row that has already been promoted to {@code NOTIFIED} (a held slot
     * awaiting confirmation) is left untouched, since that must be released through the
     * confirm/decline path instead. leaving a plain WAITING entry frees nothing, so no promotion
     * cascade is triggered.
     *
     * @param wlId      the {@code waiting_list} row id to remove
     * @param visitorId the requesting visitor's government ID (ownership guard)
     * @return {@code true} if a WAITING row was removed; {@code false} if none matched
     */
    public synchronized boolean leaveWaitingList(int wlId, String visitorId) {
        if (connection == null) return false;
        try (PreparedStatement st = connection.prepareStatement(
                "DELETE FROM waiting_list " +
                "WHERE id = ? AND visitor_id = ? AND COALESCE(status, 'WAITING') = 'WAITING'")) {
            st.setInt(1, wlId);
            st.setString(2, visitorId);
            return st.executeUpdate() > 0;
        } catch (SQLException e) {
            System.out.println("leaveWaitingList error: " + e.getMessage());
            return false;
        }
    }
